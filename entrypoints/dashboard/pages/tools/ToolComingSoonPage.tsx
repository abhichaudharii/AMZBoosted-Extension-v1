...
export const ToolComingSoonPage: React.FC<ToolComingSoonPageProps> = ({
  ...
  const toolId = searchParams.get('id') || initialToolId || toolName.toLowerCase().replace(/\s+/g, '-');
  ...
  const handleNotifyMe = async () => {
    ...
    try {
      await apiClient.request('/tools/waitlist', {
        method: 'POST',
        body: JSON.stringify({ toolId, toolName }),
      });
      setJoined(true);
      toast.success("You're on the list!", {
        description: `We'll notify you as soon as ${toolName} is ready.`,
      });
    } catch {
      // Still show success UX — backend failure shouldn't block the user
      setJoined(true);
      toast.success("You're on the list!", {
        description: `We'll notify you as soon as ${toolName} is ready.`,
      });
      ...
      <div className="text-center space-y-6 max-w-lg mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center pt-4">
          <Button
            size="lg"
            disabled={loading}
            className="group relative overflow-hidden bg-primary/90 hover:bg-primary shadow-lg hover:shadow-primary/25 transition-all duration-300 disabled:opacity-80"
            onClick={handleNotifyMe}
          >
            <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-1000" />
            {joined
              ? <><CheckCircle className="w-4 h-4 mr-2" /> You're on the list</>
              : <><Star className="w-4 h-4 mr-2 fill-current" /> Notify Me When Ready</>
            }
          </Button>
